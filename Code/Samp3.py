...
##YOU STILL NEED TO FIX THE PROBLEM WITH IT BEING TOO SMALL##
...
if args.datFile:
	...
	with open(args.datFile) as inFile, open(outName, 'a') as outFile:
		...
		subSamp = ((seq.name, seq.seq) for seq in random.choices(list(seqs), k=args.numSeq))
		crntLen=0
		for newSeq in subSamp:
			crntLen=crntLen+1
			outFile.writelines(">{}\n{}\n".format(*newSeq))
			if args.totalLen!=args.numSeq:
				random.seed(args.setSeed+1)
